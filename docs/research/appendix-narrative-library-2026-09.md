...
> 本文件是叙事库 `narratives/*.yaml` 的索引，面向程序与 AI 分析师，含卡片 id、状态枚举与映射表。面向读者的调研报告是 `docs/research/gold-pricing-logic-2026-09.md`。文中“体制 A/B”指叙事卡字段 regime_dependency 的两个取值：A 为联储被动或财政主导，B 为联储主动且可信。

# 黄金叙事库 · 第 0 阶段合并记录（2026-09）
